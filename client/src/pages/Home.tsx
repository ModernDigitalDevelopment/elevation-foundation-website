...
import { ArrowRight, Shield, Zap, Globe, ChevronDown, Sun, Vote, TrendingUp, Github, ExternalLink, FileText, BookOpen } from "lucide-react";
...
function PlainEnglishSection() {
  ...
  const extData = {
    ...
    datasets: [{
      ...
      data: [28, 34, 18, 14, 6],
      backgroundColor: [
        "rgba(192,57,43,0.8)", "rgba(192,57,43,0.85)",
        "rgba(192,57,43,0.6)", "rgba(100,100,120,0.5)", "rgba(77,184,184,0.5)",
      ],
      borderColor: [CRIMSON_CHART, CRIMSON_CHART, CRIMSON_CHART, "#666", TEAL_CHART],
  ...
  const sotData = {
    ...
    datasets: [{
      ...
      data: [40, 25, 15, 12, 8],
      backgroundColor: [
        "rgba(77,184,184,0.8)", "rgba(77,184,184,0.65)",
  ...
  const opts = {
    ...
    scales: {
      x: {
        max: 48,
        ticks: { callback: (v: any) => `${v}¢`, color: "rgba(255,255,255,0.45)", font: { size: 11 } },
  ...
  return (
    <section className="py-24 bg-navy" aria-labelledby="explainer-heading">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <div className="space-y-5 font-body text-white/65 text-base leading-relaxed">
              ...
              <div className="bg-[oklch(0.16_0.05_265)] border border-gold/20 p-5 rounded-sm">
                <p className="text-white/80 font-semibold mb-2">The old system extracts. The new system circulates.</p>
                <p className="text-white/55 text-sm">
                  In the current economy, $1 of value generated by a community produces only 6¢ of benefit for that community. The rest flows upward to executives, shareholders, and tax shelters. Sotilitarianism routes 40¢ directly to contributors — automatically, on-chain, every time.
                </p>
              </div>
              <p>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link
                href="/capitalism-2-0"
                className="inline-flex items-center gap-2 px-6 py-3 bg-gold text-[oklch(0.12_0.05_265)] font-body font-semibold text-sm rounded-sm hover:bg-gold-light transition-all group"
              >
                What is Capitalism 2.0? <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link
                href="/sotilitarianism"
                className="inline-flex items-center gap-2 px-6 py-3 border border-white/20 text-white/60 font-body text-sm rounded-sm hover:border-gold/30 hover:text-gold transition-all"
              >
                Deep Technical Dive
              </Link>
            </div>
          ...
          <div>
            <div className="bg-[oklch(0.16_0.05_265)] border border-white/10 rounded-sm p-6">
              ...
              <div className="flex gap-2 mb-5">
                {(["ext", "sot"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => setMode(m)}
                    className={`flex-1 py-2 px-3 font-mono-data text-xs uppercase tracking-wide rounded-sm border transition-all duration-200 ${
                      mode === m
                        ? m === "ext"
                          ? "bg-crimson/80 border-crimson/60 text-white"
                          : "bg-teal/80 border-teal/60 text-[oklch(0.12_0.05_265)]"
              ...
              <div className={`mt-4 p-3 rounded-sm border-l-2 text-xs font-body leading-relaxed ${
                mode === "ext"
                  ? "bg-crimson/5 border-crimson/50 text-white/50"
                  : "bg-teal/5 border-teal/50 text-white/55"
              }`}>
                {mode === "ext"
...
export default function Home() {
  return (
    <div className="min-h-screen bg-navy text-white">
        ...
        <div className="container">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              ...
              <Link
                href="/capitalism-2-0"
                className="inline-flex items-center gap-2 px-7 py-3.5 border border-white/15 text-white/55 font-body text-sm rounded-sm hover:border-gold/30 hover:text-gold transition-all duration-200"
              >
      ...
      {/* --- PLAIN-ENGLISH EXPLAINER + CHART -------------------- */}
      <PlainEnglishSection />

      {/* --- FOUNDER STORY TEASER ------------------------------- */}
      <section className="py-20 bg-[oklch(0.14_0.05_265)]" aria-labelledby="founder-heading">
        <div className="container">
          <div className="grid md:grid-cols-2 gap-14 items-center max-w-5xl mx-auto">
            <div>
              <div className="section-label mb-4">The Founder</div>
              <h2 id="founder-heading" className="font-display text-3xl md:text-4xl font-bold text-white leading-tight mb-5">
                Built by Someone Who Needed It.
                <br />
                <span className="text-gold italic">Not Just Someone Who Imagined It.</span>
              </h2>
              ...
              <p className="font-body text-white/55 text-sm leading-relaxed mb-8">
                <em>"I didn't read about communities being locked out of finance. I was in those communities. That's not a credential — it's a responsibility."</em>
              </p>
              <Link
                href="/our-story"
                className="inline-flex items-center gap-2 text-gold font-body font-medium hover:gap-3 transition-all duration-200 group"
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={item.label} className="flex items-start gap-4 p-4 bg-[oklch(0.16_0.05_265)] border border-white/10 rounded-sm">
                  <div className="font-mono-data text-xs text-gold/40 w-5 flex-shrink-0 mt-0.5">{String(i + 1).padStart(2, "0")}</div>
                  <div>
                    <div className="font-body font-semibold text-gold text-xs uppercase tracking-wide mb-1">{item.label}</div>
                    <div className="font-body text-white/55 text-sm leading-relaxed">{item.text}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* --- SSRN CITATION STRIP -------------------------------- */}
      <section className="py-12 bg-navy border-y border-white/10" aria-label="Academic citation">
        <div className="container">
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <div className="flex-shrink-0">
              <div className="w-14 h-14 flex items-center justify-center bg-gold/10 border border-gold/30 rounded-sm">
                <FileText size={22} className="text-gold" />
              </div>
            </div>
            <div className="flex-1">
              <div className="font-mono-data text-[10px] uppercase tracking-wider text-gold/60 mb-1">Peer-Reviewed · SSRN · May 2026</div>
              ...
              <div className="font-body text-white/45 text-sm">Cornelius DeFalco · Social Science Research Network · 19 pages</div>
            </div>
            <a
              href={SSRN_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="flex-shrink-0 inline-flex items-center gap-2 px-5 py-2.5 border border-gold/30 text-gold font-body text-sm font-medium rounded-sm hover:bg-gold/10 transition-all duration-200 whitespace-nowrap"
            >
